...
def hex_to_tuple(text):
    ...
    rgba=(pow(int(r,16)/255, gamma), pow(int(g,16)/255, gamma), pow(int(b,16)/255, gamma), 1)
    return rgba
...
def 给cube添加材质(timelinesJsonData):
    ...
    mulrgb.inputs[1].default_value = (1, 1, 1, 1)
...
#物体名称字典
NameDict={}
def spawn_timelines_obj(MiObjectJson, 恢复命名):
    #创建物体
