...
def SkinConverting(file):
    ...
    if img.size[0] == img.size[1]:
        ...
        img_new.paste(arm1_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*4,基础长度*12))
        
        arm2_img = img.crop((基础长度,基础长度 * 4,基础长度 * 2,基础长度*5))
        img_new.paste(arm2_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*5,基础长度*11))
        
        arm3_img = img.crop((基础长度 * 2,基础长度 * 4,基础长度 * 3,基础长度*5))
        img_new.paste(arm3_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*6,基础长度*11))
        
        arm4_img = img.crop((基础长度 * 3,基础长度 * 5,基础长度 * 4,img_size[1]))
        img_new.paste(arm4_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*7,基础长度*12))
        
        # 腿翻转粘贴
        leg1_img = img.crop((基础长度 * 10,基础长度 * 5,基础长度 * 13,img_size[1]))
        img_new.paste(leg1_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*8,基础长度*12))
        
        leg2_img = img.crop((基础长度 * 11,基础长度 * 4,基础长度 * 12,基础长度 * 5))
        img_new.paste(leg2_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*9,基础长度*11))
        
        leg3_img = img.crop((基础长度 * 12,基础长度 * 4,基础长度 * 13,基础长度 * 5))
        img_new.paste(leg3_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*10,基础长度*11))
        
        leg4_img = img.crop((基础长度 * 13,基础长度 * 5,基础长度 * 14,基础长度 * 9))
        img_new.paste(leg4_img.transpose(Image.FLIP_LEFT_RIGHT), (基础长度*11,基础长度*12))
